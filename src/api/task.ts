...
export const approveTaskRequest = async (requestId: number, data: ApprovalRequestBody) => {
  // 타입 지정
  return apiRequest<ApprovalResponse>('POST', API_ENDPOINTS.APPROVE_REQUEST(requestId), data);
}
...
export const updateTaskRequest = async (requestId: number, data: UpdateTaskRequestData) => {
...
export const rejectTaskRequest = async (requestId: number, data: RejectionRequestBody) => {
  // FormData 대신 JSON 형식으로 요청
  return apiRequest<ApprovalResponse>('POST', API_ENDPOINTS.REJECT_REQUEST(requestId), data);
};
...
export const updateTaskResponse = async (responseId: number, data: {
  comment: string;
  projectId: number;
  links: { urlAddress: string; urlDescription: string }[];
}) => {
...
export const uploadResponseFiles = async (responseId: number, files: File[]) => {
  const formData = new FormData();
...
export const getRequestResponses = async (requestId: number) => {
  return apiRequest<ApprovalResponse[]>('GET', `/requests/${requestId}/responses`)
} 